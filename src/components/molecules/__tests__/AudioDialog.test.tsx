...
describe("AudioDialog", () => {
    it("renders without showing dialog initially", () => {
        const { queryByText } = renderWithProvider(<AudioDialog auroraSoundList={[]} />);
        expect(queryByText("A New Day")).toBeNull();
    });

    it("shows dialog after AudioDialog.show() is called", async () => {
        const onConfirm = jest.fn();
        const { getByText } = renderWithProvider(<AudioDialog auroraSoundList={[]} />);

        await act(async () => {
            AudioDialog.show({ onConfirm }, AudioList.A_NEY_DAY);
        });
